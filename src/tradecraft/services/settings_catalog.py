...
META: dict[str, SettingMeta] = {
    ...
    "llm_model": SettingMeta(
        "핵심 판단 모델",
        "KIS·시장판단처럼 실제 거래에 직접 영향을 주는 핵심 모델입니다.",
        "ai",
        choices=("gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna", "gpt-5.5"),
    ),
    "llm_reasoning_effort": SettingMeta(
        "추론 강도",
        "실시간 핵심 판단의 reasoning effort입니다. 지연을 고려해 xhigh를 기본으로 사용합니다.",
        "ai",
        choices=("low", "medium", "high", "xhigh"),
    ),
    "llm_reasoning_model": SettingMeta(
        "분석 모델",
        "리서치·종목분석·메모리 종합에 쓰는 균형형 모델입니다.",
        "ai",
        choices=("gpt-5.6-terra", "gpt-5.6-sol", "gpt-5.6-luna", "gpt-5.5"),
    ),
    "llm_reasoning_model_effort": SettingMeta(
        "분석 추론 강도",
        "리서치와 메모리 종합의 reasoning effort입니다.",
        "ai",
        choices=("low", "medium", "high", "xhigh"),
    ),
    "llm_utility_model": SettingMeta(
        "정리·추출 모델",
        "보고서 사실 추출과 반복 요약·분류에 쓰는 대량 처리 모델입니다.",
        "ai",
        choices=("gpt-5.6-luna", "gpt-5.6-terra", "gpt-5.6-sol", "gpt-5.5"),
    ),
    "llm_utility_model_effort": SettingMeta(
        "정리·추출 강도",
        "정형 추출과 압축 작업의 reasoning effort입니다.",
        "ai",
        choices=("low", "medium", "high"),
    ),
    "llm_offline_model": SettingMeta(
        "오프라인 최고 판단 모델",
        "주·월간 정책 개정과 코드랩처럼 지연보다 깊이가 중요한 작업의 모델입니다.",
        "ai",
        choices=("gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna", "gpt-5.5"),
    ),
    "llm_offline_model_effort": SettingMeta(
        "오프라인 추론 강도",
        "실시간 주문에는 적용하지 않는 오프라인 최고 추론 강도입니다.",
        "ai",
        choices=("high", "xhigh", "max"),
    ),
    "codex_runtime_timeout_ms": SettingMeta(
        "LLM 타임아웃(ms)",
        "Codex native runtime 응답을 기다리는 최대 시간입니다. Sol xhigh 장중 판단은 길게 기다립니다.",
        "ai",
        min_value=1000,
        max_value=600000,
        step=1000,
    ...
    "binance_block_trader_waiting_entry_max_age_sec": SettingMeta(
        "바이낸스 대기진입 만료(초)",
        "proposed 대기진입 블록이 이 시간을 넘으면 executor가 자동으로 닫아 오래된 후보가 신규 판단을 가리지 않게 합니다. 0이면 비활성화합니다.",
        "trading",
        min_value=0,
        max_value=604800,
        step=3600,
    ...
    "binance_block_trader_entry_pending_max_age_sec": SettingMeta(
        "바이낸스 미체결 진입 해제(초)",
        "entry_pending 진입 주문이 체결 증거와 계정 포지션 없이 이 시간을 넘으면 executor가 슬롯을 해제합니다. 0이면 비활성화합니다.",
        "trading",
        min_value=0,
        max_value=86400,
        step=60,
