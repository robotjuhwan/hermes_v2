...
def build_kis_operational_retention_rules(
    *,
    ...
    if int(reconciliation_retention_days) > 0:
        rules.append(
            RetentionRule(
                ...
                timestamp_column="run_at",
                retention_days=int(reconciliation_retention_days),
                archive_table="reconciliation_runs_archive",
                archive_compress_columns=("account_json", "summary_json"),
                vacuum_after_delete=True,
            )
        )
    if int(archive_retention_days) > 0:
        for table, timestamp_column, source_retention_days in (
            ...
            (
                ...
                "run_at",
                int(reconciliation_retention_days),
            ),
        ):
            rules.append(
                RetentionRule(
