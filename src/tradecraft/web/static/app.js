...
function renderHelperAgent() {
  ...
  const validTabs = new Set(["research", "runtime", "rebalance", "kis_trader", "reports"]);
  ...
  if (state.activeHelperTab === "runtime") {
    ...
    contentRoot.innerHTML = renderRebalanceHelperTab(state.rebalanceStatus, state.rebalanceError);
...
async function refreshDashboard() {
  const [dashboardResult, kisResult, reportsResult, healthResult, rebalanceResult] = await Promise.allSettled([
    getJSON("/dashboard"),
